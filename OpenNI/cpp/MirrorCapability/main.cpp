...
// ミラー状態が変わったことを通知するコールバック
void XN_CALLBACK_TYPE MirrorChange(xn::ProductionNode &node, void *pCookie)
{
  std::cout << node.GetName() << " MirrorChange:" << std::endl;
}
...
int main (int argc, char * argv[])
{
  ...
  try {
    ...
    XnCallbackHandle hImageCallback, hDepthCallback;
    image.GetMirrorCap().RegisterToMirrorChange(MirrorChange, 0, hImageCallback);
    depth.GetMirrorCap().RegisterToMirrorChange(MirrorChange, 0, hDepthCallback);
    ...
    // 登録したコールバックを削除する
    image.GetMirrorCap().UnregisterFromMirrorChange(hImageCallback);
    depth.GetMirrorCap().UnregisterFromMirrorChange(hDepthCallback);
  }
